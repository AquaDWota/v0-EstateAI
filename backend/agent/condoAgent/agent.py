...
# Config
# ----------------------------
subject_matter = "Condo Specialist"
...
You are a conservative real estate investment analyst specializing in CONDO rental properties.

You will be given a list of condo listings (each may include address, id, zipCode, listPrice, estimatedRent, bedrooms, bathrooms, sqft, yearBuilt, propertyTaxPerYear, insurancePerYear, hoaPerYear).
...
- HOA burden: <$X/year or low / moderate / high>

Risk & downside analysis:
<1–2 sentences addressing HOA escalation, assessment risk, rental restrictions, and margin fragility.>
...
CONDO INVESTMENT LOGIC:
- HOA fees materially reduce net yield
- Special assessments and reserve risk are critical
- Rental restrictions must always be flagged
- Risk-adjusted cashflow matters more than amenities
...
STYLE REQUIREMENTS:
- Clear, conservative, investor-oriented
- No JSON, no emojis, no math shown

Now evaluate the condo listings provided in the user prompt and return ONLY the top 5 ranked investments using the format above.
"""
...
def run_condo_analysis(user_text: str) -> str:
...
agent = Agent(
    ...
    seed=os.getenv("CONDO_SEED", "CondoSpecialist2026"),
    port=int(os.getenv("CONDO_PORT", "8012")),
    mailbox=True,
...
async def handle_chat(ctx: Context, sender: str, msg: ChatMessage):
    ...
    if any(isinstance(item, StartSessionContent) for item in msg.content):
        await ctx.send(sender, create_text_chat(f"Hi! I'm a {subject_matter}. Send condo listings to analyze."))
        return
    ...
    # Router sends JSON: {"type":"estate_specialist_request","request_id":"...","specialist":"condo","user_text":"..."}
    try:
        data = json.loads(text)
        if isinstance(data, dict) and data.get("type") == "estate_specialist_request":
            request_id = data.get("request_id")
            specialist = data.get("specialist") or "condo"
            user_text = data.get("user_text", "")

            try:
                analysis = run_condo_analysis(user_text)
    ...
    try:
        response = run_condo_analysis(text)
    except Exception as e:
        ctx.logger.exception("Error querying ASI model")
...
async def handle_router_req(ctx: Context, sender: str, msg: SpecialistRequest):
    ctx.logger.info(f"[CondoSpecialist] got request_id={msg.request_id}")

    try:
        result_json_text = run_condo_analysis(msg.user_text)
    except Exception as e:
    ...
    await ctx.send(
        ...
        SpecialistResult(
            ...
            specialist=msg.specialist,  # should be "condo"
